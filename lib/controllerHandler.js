	...
	* On success, it starts the EventSource connection and emits the initial state of all items. */
	...
	async _publishStateOfItems() {
		...
		for (const element of itemsResponse.data) {
			// Simulate an event. Note that type is different than what the events return. This is an inconsistency in OpenHAB that we can't easily fix.
			const message = {topic: `openhab/items/${element.name}/state`, type: EVENT_TYPES.ITEM_STATE, payload: { type: element.type, value: element.state } };
			this._publishReceivedEvent(message);
		};
	}
...
;		if (!match) return;
		...
		let message = {
			...
			concept,
			name,
			full_name: EVENT_TAGS.FULL_NAME(concept, name),
			...(event && { event }),
		};
		this.eventBus.publish(message.full_name, message);
	}
