...
const { CONCEPTS, EVENT_TAGS, EVENT_TYPES, OPERATION, STATE, SWITCH_STATUS, HTTP_METHODS } = require('./constants');
...
class ControllerHandler {
    ...
    async control(resource, operation = OPERATION.GET, payload = null) {
        ...
        const response = await this.connection.sendRequest(parameters.endPoint, parameters.verb, payload);
    ...
    async getResources(type, endpoint) {
        console.log(`ControllerHandler: Fetching ${type} at ${endpoint}`);
        return await this.connection.getResources(type, endpoint);
    }
    ...
    async _getAll(conceptType) {
        ...
        const response = await this.connection.sendRequest(endPoint, HTTP_METHODS.GET, null);
        if (!response.ok) {
            // inform the listeners of errors. We can do that here, since controllerHandler is initiator.
            this._publishError({ function: '_getAll', endPoint, operation }, response);
        }
        return response;
    }
    ...
    _handleMessage = (event) => {
        ...
        if (!msg) {
            this._publishError({ function: '_handleMessage', event }, 'Failed to parse incoming event as JSON');
            return;
        }
        ...
        try {
            switch (state) {
                ...
                case STATE.CONNECTING:
                    ...
                    this._publishError({ function: '_handleStateChange' }, 'Connection lost. Reconnecting...');
                    this.eventBus.publish(EVENT_TAGS.CONNECTION_STATUS, SWITCH_STATUS.OFF);
                    break;
            ...
            // Keep it minimal but safe: log error
...
function setupControllerHandler(node, config, options = {}) {
    return new ControllerHandler(node, config, options).setupNode();
}
