...
const { CONCEPTS, EVENT_TAGS, EVENT_TYPES, OPERATION, STATE, SWITCH, HTTP_METHODS } = require('./constants');
...
class ControllerHandler {
    ...
    async _getAll(conceptType) {
        ...
        if (!response.ok) {
            ...
            this._publishError(
                { function: '_getAll', endPoint, operation, response },
    ...
    _parseResponse(conceptType, response) {
        // don't do anything with error responses
    ...
    _publishConnectionStatus(payload) {
        const message = { payload, topic: EVENT_TAGS.CONNECTION_STATUS, payloadType: 'Switch' };
        this.eventBus.publish(EVENT_TAGS.CONNECTION_STATUS, message);
    }
