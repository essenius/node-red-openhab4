...
class InNode extends ConsumerNodeBase {
    ...
    setupNodeLogic(options = {}) {
        ...
        this.refreshNodeStatus(this.node.context().get("currentState"));
    }

    _setupItemListeners() {
        // Bind event handler methods to maintain 'this' context
        this._processStateEvent = this._processStateEvent.bind(this);
        this._processRawEvent = this._processRawEvent.bind(this);

        const stateEventName = this.itemName + '/' + EVENT_TYPES.STATE_EVENT;
        const rawEventName = this.itemName + '/' + EVENT_TYPES.RAW_EVENT;

        this.controller.on(rawEventName, this._processRawEvent);
        this.controller.on(stateEventName, this._processStateEvent);
    }
    ...
    cleanup() {
        ...
        if (this.controller) {
            this.controller.off(this.itemName + '/' + EVENT_TYPES.STATE_EVENT, this._processStateEvent);
            this.controller.off(this.itemName + '/' + EVENT_TYPES.RAW_EVENT, this._processRawEvent);
        }
    }
}
