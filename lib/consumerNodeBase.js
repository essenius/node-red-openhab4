...
class ConsumerNodeBase {
    ...
    setupNode() {
        ...
        if (!this.controller) {
            ...
            this.setStatusError("no controller");
        ...
        this.setStatusWaiting("Waiting for openHAB");
    ...
    setStatus(state, text = "") {
        ...
        this.node.status({
            fill: mapping.fill,
            shape: mapping.shape,
            text: this.truncateMessage(text || "unknown")
        });
    }
    ...
    // convenience methods 
    setStatusInit(text = "initializing") { this.setStatus(STATE.INIT, text); }
    setStatusReady(text = "ready") { this.setStatus(STATE.READY, text); }
    setStatusWaiting(text = "waiting...") { this.setStatus(STATE.WAITING, text); }
    setStatusWorking(text = "working...") { this.setStatus(STATE.WORKING, text); }
    setStatusConnected(text = "connected") { this.setStatus(STATE.CONNECTED, text); }
    setStatusOK(text = "") { this.setStatus(STATE.OK, text); }
    setStatusOKFalsy(text = "") { this.setStatus(STATE.OK_FALSY, text); }
    setStatusWarning(text = "warning") { this.setStatus(STATE.WARNING, text); }
    setStatusError(text = "error") { this.setStatus(STATE.ERROR, text); }
    setStatusDisconnected(text = "disconnected") { this.setStatus(STATE.DISCONNECTED, text); }
}
