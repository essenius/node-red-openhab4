...
function setupOpenhabController(node, config) {
	// Add semantic status methods to the node
	addStatusMethods(node);

	let stateRetryTimer = null;
	let connection = null;
	let startupTimer = null; // Track the startup timer!
	...
	async function getStateOfItems() {
		try {
			...
			node.emit(EVENT_TYPES.COMMUNICATION_STATUS, SWITCH_STATUS.ON);
		...
		try {
			...
			if (typeof msg.payload === "string" && msg.payload.trim() !== "") {
				try {
					...
					node.warn(`Failed to parse event payload as JSON: ${msg.payload}`);
	...
	// Start the centralized connection with controller-specific options
	startupTimer = setTimeout(function () {
		startupTimer = null; // Clear reference when timer executes
		
		// Safety check: don't start connection if node is being destroyed
		if (!node || node._closed) {
			node.log('🛑 Startup timer cancelled - node is closed');
			return;
		}
		
		node.log("Starting EventSource connection to openHAB");
		connection.startEventSource({
			topics: EVENT_TOPICS.ALL_ITEMS,
			onOpen: () => {
				// Get the current state of all items when connection opens
				if (node && !node._closed) {
					getStateOfItems();
				}
			},
	...
	node.on("close", function (removed, done) {
		...
		// Clear startup timer to prevent connection after deletion
		if (startupTimer) {
			node.log('🔥 Clearing startup timer');
...
module.exports = { setupOpenhabController };
