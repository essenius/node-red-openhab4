...
class EventsNode extends ConsumerNodeBase {
    ...
    processRawEvent(event) {
        ...
        this.node.send(rawMsg);
    }

    setupNodeLogic(options = {}) {
        console.log("EventsNode setupNodeLogic called with options:", options );
        if (options.error) return;
        this.processRawEvent = this.processRawEvent.bind(this);
        this.controller.on(EVENT_TYPES.RAW_EVENT, this.processRawEvent);
        console.log("EventsNode setupNodeLogic completed");
    }
...
*/
    ...
    cleanup() {
        ...
        console.log("EventsNode cleanup called");
        this.controller.off(EVENT_TYPES.RAW_EVENT, this.processRawEvent);
        super.cleanup();
    }
}
