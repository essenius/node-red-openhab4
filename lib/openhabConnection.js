...
class OpenhabConnection {
    ...
    startEventSource() {
        ...
        this.eventSource = new this.dependencies.eventSourceImpl(url, esOptions);
        this.eventSource.onopen = async () => this._enterUp();
        this.eventSource.onmessage = async (event) => this.dependencies.onMessage(event);
    ...
    async _probeHealth() {
        ...
        const result = await this.dependencies.httpRequest(probeUrl, this.config, { method: HTTP_METHODS.GET });
