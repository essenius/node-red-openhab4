...
class OpenhabConnection {
    ...
    async controlItem(name, topic, payload) {
        console.log(`Controlling item: ${name}, topic: ${topic}, payload: ${payload}`);
        const topicConfig = {
        ...
        console.log(`Calling fetchOpenHAB with url: ${url}, method: ${config.method}`);
        const result = await fetchOpenHAB(url, this.config, {
        ...
        console.log(`fetchOpenHAB result: ${JSON.stringify(result)}`);
