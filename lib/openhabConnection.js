...
class OpenhabConnection {
    ...
    async sendRequest(endPoint, method = HTTP_METHODS.GET, payload = null) {
        ...
        if (this.state !== STATE.UP) return { ok: false, type: ERROR_TYPES.SYSTEM, retry: true, message: `${OPENHAB} offline` };
    ...
    async _probeHealth() {
        ...
        const result = await httpRequest(probeUrl, this.config, { method: HTTP_METHODS.GET });
        return result.ok === true;
    }

    _retryMessage(message, errorType = ERROR_TYPES.SYSTEM) {
        return { ok: false, type: errorType, retry: true, message };
    }
