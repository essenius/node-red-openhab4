...
const { ENDPOINTS, CONTROL_TOPICS, RETRY_CONFIG, HTTP_METHODS, ERROR_TYPES, EVENT_TAGS } = require("./constants");
const { httpRequest, getConnectionString, getEventSource, isPhantomError, responseStatus, retryable } = require("./connectionUtils");
...
class OpenhabConnection {
    ...
    async getItems(errorHandler) {
        ...
        return await retryable(() => httpRequest(url, this.config), { errorFunction: errorHandler });
    }
    ...
    async controlItem(name, topic, payload, errorHandler) {
        ...
        const config = topicConfig[topic] || {
            endpoint: name == "" ? ENDPOINTS.TEST_URL : ENDPOINTS.ITEM_COMMAND(name),
        ...
        // if name is empty, this was a "ping". Return server metadata.
        if (name == "" && result.ok) result.data = this.extractMetadata(result.data);
        return result;
    }

    extractMetadata(data) {
        let result = { version: data.version };
        if (data.runtimeInfo) {
            result.runtimeInfo = data.runtimeInfo;
        } else {
        ...
        if (data.measurementSystem) {
            result.measurementSystem = data.measurementSystem;
        }
        return result;
    }

    buildEventSourceUrl(topics) {
        let url = getConnectionString(this.config) + ENDPOINTS.EVENTS;
    ...
    handleEventSourceError(error, options, onError) {
        ...
        // Parse error into a standardized structure
        let response = {};
        if (error?.type) {
            ...
            response.status = 500; // default Internal Server Error
            response.message = typeof error === "string" ? error : String(error);
        }
    ...
    /*
    startEventSource1(options = {}) {
        const { onOpen = () => { }, onMessage = () => { }, onError = () => { }, topics } = options;

        // Close any existing EventSource before starting a new one
        this.close(onError);

        // For SSE, we no longer have to include credentials in the URL. Limitation lifted in 3.0.
        let url = getConnectionString(this.config) + ENDPOINTS.EVENTS;
