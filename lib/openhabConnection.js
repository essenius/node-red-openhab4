...
const { RETRY_CONFIG, HTTP_METHODS } = require("./constants");
...
class OpenhabConnection {
    ...
    /** Control an OpenHAB resource by sending a command, or getting/updating its state.
     * If an error occurs, the error handler is called, and if possible a retry is attempted.  */
    async sendRequest(endPoint, method = HTTP_METHODS.GET, payload = null, errorHandler = () => {}) {
        const url = this.config.url + endPoint;
        const result = await retryable(
            () => httpRequest(url, this.config, {
                method,
                body: payload ? String(payload) : undefined
            }), 
    ...
    handleEventSourceError(error, options, onError) {
        ...
        onError(`${result.message} (${shortMessage})`);
    ...
    startEventSource(options = {}) {
        const { onOpen, onMessage, onError, endPoint } = {
            onOpen: () => { },
            onMessage: () => { },
            onError: () => { },
            ...options
    ...
    close(onError = () => { }) {
        ...
        if (this.eventSource) {
            try {
                ...
                // not a big deal usually, but we should report it. 
