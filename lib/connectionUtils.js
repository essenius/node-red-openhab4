...
function getErrorMessage(response, data, status) {
    console.log("getErrorMessage called with response:", response, "data:", data, "status:", status);
...
async function fetchOpenHAB(url, config, options = {}) {
    ...
    try {
        ...
        return { status: response?.status || 500, error, message: error.message || response?.statusText || "Fetch failed" };
    }
    console.log("Response:", response.status, response.statusText, "Data:", data);
