...
async function httpRequest(url, config, options = {}) {
    ...
    if (hasCredentials) {
        const auth = Buffer.from(`${config.username}:${_setWithDefault(config.password, "")}`).toString("base64");
...
function setDefaults(config) {
    ...
    config.protocol = _setWithDefault(config.protocol, "http");
    config.host = _setWithDefault(config.host, "localhost");
    config.port = _setWithDefault(config.port, config.protocol === "https" ? 8443 : 8080);
    config.path = _trimSlashes(_setWithDefault(config.path, ""));
...
const _TLS_ERRORS = new Set(['ERR_TLS_CERT_ALTNAME_INVALID', 'ERR_TLS_CERT_EXPIRED']);
...
function _setWithDefault(property, defaultValue, options = {}) {
    ...
    if (typeof property === "number") {
        return Number.isNaN(property) ? defaultValue : property;
    }
