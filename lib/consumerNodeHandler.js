...
class ConsumerNodeHandler {
    ...
    getConceptType(concept) {
        ...
        if (!conceptType) {
            this.setStatus(STATE.ERROR, concept + ": unknown concept");
            return null;
        }
        return conceptType;
    }
    ...
    setValueStatus(state) {
        ...
        // also check if the value is "OFF", "OFFLINE" or starts with "OFF ", which is also considered falsy
        isFalsy = isFalsy || !state || /^OFF($| |LINE$)/.test(state);
    ...
    parseTopic(topic) {
        if (typeof topic !== "string") return { valid: false };
        ...
        const identifier = rest.length ? rest.join("/") : undefined;
        return {
            valid: this.validateConcept(concept),
            concept,
            identifier
        };
    }
    ...
    validateConcept(concept) {
        ...
        const validConcepts = Object.values(CONCEPTS);
        return validConcepts.includes(concept);
    }
    ...
    _truncateMessage(message, maxLength = 35) {
