...
class ConsumerNodeHandler {
    constructor(node, config, controller, utils = {}) {
        ...
        this.eventBus = controller?.eventBus;
        this.utils = utils;
    }
    ...
    createMessage({ message, ...properties }) {
        ...
        if (message) {
            ...
            if (!properties.payload || !properties.topic) {
                throw new Error("createMessage requires either message or payload and topic properties");
            }
    ...
    capitalizeFirst(value) {
        if (!value) return value; // handle empty string
        return value.charAt(0).toUpperCase() + value.slice(1);
    }
    /** Returns the type of the node, used for names, logging and debugging
     * We use the type that was set in the node registration without the prefix
    ...
    handlerOff(eventTag, handler) {
        if (this.eventBus) {
            this.eventBus.unsubscribe(this.node, eventTag);
            this.node.off(eventTag, handler);
        }
    }
    ...
    handlerOn(eventTag, handler) {
        if (this.eventBus) {
            this.node.on(eventTag, handler);
            this.eventBus.subscribe(this.node, eventTag);
        }
    }
