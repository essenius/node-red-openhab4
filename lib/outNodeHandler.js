...
class OutNodeHandler extends ConsumerNodeHandler {
    ...
    async handleInput(msg) {
        ...
        const operation = this.prioritizedProperty(msg.openhabControl?.operation, this.config.operation)
            .trim()
            .toLowerCase();
        ...
        if (result.ok) {
            ...
            const message = {
                ...
                topic: `${item.concept}/${item.identifier}`, // override topic with the one actually used
            ...
            this.handleSendError(result.code || 'send failed', `${payload} ✗`);
        }
    }
