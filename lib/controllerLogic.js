...
function handleControllerError(node, error, source = null) {
    const message = (source === null ? "" : (source + ": ")) + ((error && error.message) ? error.message : String(error));
    node.error(message);
	node.emit(EVENT_TYPES.COMMUNICATION_ERROR, message);
}
...
function setupControllerNode(node, config, options = {}) {
	...
	(async () => {
		...
		if (!ready) {
			handleControllerError(node, new Error("Timeout"), "Waiting for openHAB");
			return;
		}
	...
	node.control = async function (itemname, topic = null, payload = null, consumerNode = null) {
		try {
			return await connection.controlItem(itemname, topic, payload);
		} catch (error) {
			handleControllerError(node, error);
			return error;
		}
	};
	...
	function handleControllerMessage(event) {
		...
		try {
			...
			if (node && !node._closed) {
				handleControllerError(node, e, "Parsing event");
			}
		}
	}
}
