...
function setupControllerNode(node, config, options = {}) {
	...
	(async () => {
		...
		connection.startEventSource({
			...
			onError: ifActive(node, (status, message, shortMessage) => {
				...
				shortMessage = shortMessage || `SSE error ${status}`;
				node.emit(EVENT_TYPES.CONNECTION_ERROR, shortMessage);
			}),
			onMessage: ifActive(node, handleControllerMessage),
			onWarning: ifActive(node, (message) => {
				node.warn(`[openhab4] SSE warning: ${message}`);
			})
		});
	...
	function handleControllerMessage(event) {
		try {
			...
			// Emit events with item name prepended so in nodes can listen to it
