...
class HealthNode extends ConsumerNodeBase {
    ...
    setupNodeLogic(options = {}) {
        ...
        if (options.error) {
            const message = this.createMessage({ payload: String(options.error), event: EVENT_TYPES.CONNECTION_ERROR });
            this.node.send([null, message, null]);
            return;
        }
...
module.exports = { HealthNode, setupHealthNode };