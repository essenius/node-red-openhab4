...
describe("getLogic handleInput", function () {
    ...
    it("should show an error if no item is specified", async function () {
        ...
        const getNode = new GetNode(node, config, controller, { generateTime: () => "12:34:56" });
    ...
    it("should show waiting and then error if response is unexpected", async function () {
        ...
        expect(node.status.getCall(0).args[0], "waiting status called").to.deep.equal({ fill: 'blue', shape: 'ring', text: 'requesting... @ 12:34:56' });
        expect(controller.handleControllerError.calledWith(sinon.match.instanceOf(Error), 'Get'), "Controller error called").to.be.true;
        expect(node.status.getCall(1).args[0], "error status called").to.deep.equal({ fill: 'red', shape: 'ring', text: 'Call failed @ 12:34:56' });
    });
});