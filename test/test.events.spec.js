...
/* class FakeEventSource {
    constructor(_url, _opts) {
        // immediately trigger the onmessage callback
        console.log("Created mock event source");
        setTimeout(() => {
            if (this.onmessage) {
                this.onmessage({
                    data: JSON.stringify({
...
const eventsModule = proxyquire("../nodes/events.js", {
...
describe("openhab4-events integration", function () {
    ...
    it("should send a message when an item is published", function (done) {
        const flow = [
            ...
            { id: "events1", type: "openhab4-events", controller: "controller1", wires: [["helper1"]], filter: "items/*" },
        ...
        helper.load([controllerNode, eventsModule], flow, function () {
            ...
            controller.handler.eventBus.publish("items/test1", {payload: SWITCH_STATUS.OFF, topic: "test1", type: EVENT_TYPES.ITEM_STATE} );
