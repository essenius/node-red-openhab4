...
function createConnection(configOverrides = {}, dependencyOverrides = {}) {
...
describe('openhabConnection tests', function () {
    beforeEach(() => {
    ...
    describe('OpenhabConnection sendRequest', function () {
        ...
        it('should stay UP when a message arrives', async function () {
            ...
            expect(connection.eventSourceEndPoint).to.equal('/rest/events');
            ...
            expect(connection.state).to.equal(STATE.UP);
        });
        ...
        [
            ...
            it(`should call health probe for 404/500 without retry flag and ${name}`, async function () {
                ...
                if (!secondResponse.ok) {
                    expect(response.message).to.equal('OpenHAB offline');
                }
            });
        });
    });
    ...
    describe('OpenhabConnection Retry scheduler', function () {
        it('should schedule reconnect when entering CONNECTING', async function () {
            ...
            await connection._enterConnecting();
            expect(fakeSetTimeout.calledOnce, 'Timeout() not called again').to.be.true;
        });
...
describe('openhabConnection dependency tests', function () {
    it('takes the right defaults if not specified', function () {
        ...
        expect(deps.now).to.equal(Date.now);
    });
});
