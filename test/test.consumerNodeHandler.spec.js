...
describe('consumerNodeHandler', function () {
    ...
    describe('StatusMethods', function () {
        let mockNode, mockBus, mockController, nodeHandler;
        ...
        it('retrieves type correctly', function() {
            ...
            expect(nodeHandler.getNodeType()).to.equal("type-without-prefix", "Node type without prefix determined correctly");
            nodeHandler.node = null;
            expect(nodeHandler.getNodeType(), "Node type undefined when no node").to.be.undefined;


        });
        it('registers the handlers registered with controller.on', function () {
            nodeHandler.createMessage = () => { };
            nodeHandler._setupMetaEvents();

            expect(nodeHandler.eventBus.subscribe.callCount).to.equal(2);
            expect(nodeHandler.eventBus.subscribe.firstCall.args).to.deep.equal([nodeHandler.node, EVENT_TAGS.CONNECTION_STATUS]);
            expect(nodeHandler.eventBus.subscribe.secondCall.args).to.deep.equal([nodeHandler.node, EVENT_TAGS.NODE_ERROR]);
        });
        ...
        it('should set status to init', function () {
            ...
            expect(mockNode.status.firstCall.args).to.deep.equal([{ fill: "grey", shape: "ring", text: "testing @ 12:34:56" }], "Init handled ok");
        });
