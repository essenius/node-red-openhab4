...
- ***Operation*** *(string)* — Optional. The type of operation (Command or Update).
...
- ***openhabControl*** *(object)* — object containing a property `operation` to specify the operation.

### Outputs

- **payload** *(string)* — The payload that was sent.
- **topic** *(string)* — The address of the item that received the operation.
- **openhabControl** *(object)* — the control information that was used.
- **input** *(object)*  — The original input message.

### Details
This node sends operations (commands or state updates) to openHAB items:
...
| `Item Name`   | `msg.topic`                    |
| `Payload`     | `msg.payload`                  |
| `Operation`   | `msg.openhabControl.operation` |  
